...
class Document(models.Model):
    ...
    # custom field
    # trich_yeu = fields.Char(string="Trích yếu")
    so_ki_hieu = fields.Char(string="Số ký hiệu")
    don_vi_pnat_hanh = fields.Many2one('user.info.department', string="Đơn vị ban hành")
    ngay_ban_hanh = fields.Date(string="Ngày ban hành", default=fields.Date.today())
    so_van_ban = fields.Char('Sổ văn bản')
    so_phat_hanh = fields.Char('Số phát hành')
    van_ban = fields.Char('Văn bản')
    van_ban_giay = fields.Char('Văn bản giấy')
    do_mat = fields.Char('Độ mật')
    # loai_van_ban = fields.Char('Loại văn bản')
    hinh_thuc = fields.Char('Hình thức')
    do_khan = fields.Char('Độ khẩn')
    nguoi_soan = fields.Char('Người soạn thảo')
    nguoi_ky =  fields.Many2many('user.super.admin', string='Người ký')

    name = fields.Char(string="Trích yếu", help="Document name")
    attachment = fields.Binary(string='File', help="Document data")
    date = fields.Datetime(string='Date', help="Document create date")
    workspace_id = fields.Many2one(
        'document.workspace', string='Loại văn bản',
        required=True, help="workspace name",store=True,
        default=lambda self: self.env['document.workspace'].search([('name', '=', 'Van ban di')]))
    ...
    security = fields.Selection(
        ...
        ], default='specific_users', string="Security",
